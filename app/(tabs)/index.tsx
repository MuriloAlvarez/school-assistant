...
import { SchoolCard, useSchoolStore, useSchools } from "@/features/schools";
import type { School } from "@/features/schools";

export default function SchoolsScreen() {
  const router = useRouter();
  const { schools, isLoading, fetchSchools } = useSchools();
  const { filters, setFilters } = useSchoolStore();
    ...
    <Box flex={1} bg="$backgroundLight50">
      ...
      <FlatList<School>
        ...
        renderItem={({ item }) => (
          <SchoolCard
            school={item}
            onPress={() => router.push(ROUTES.SCHOOLS.detail(item.id))}
          />
        )}
        refreshControl={
